...
Ext.define('NV.mplane.gui.Constants',{
	...
	urls:{
		...
		reasonerNetViewUrl : "/gui/reasonerNetView"
	},
...
Ext.define('NV.mplane.gui.Main',{
	...
	goToPage:function(params){
		...
		if(this.tabPanel.items.keys.indexOf(params.page)>=0){	//opened
			...
			if(NV.mplane.gui.Constants.capabilityMenu[params.page].type=='reasoner'){
				newTab = Ext.create('NV.mplane.gui.tab.ReasonerNetViewTab', {itemId:params.page});
				this.renderedTabs[params.page] = newTab;
			}
...
// @tag mplane-gui
// Invernizzi - Reasoner net view
// @tag mplane-gui
Ext.define('NV.mplane.gui.tab.ReasonerNetViewTab',{
	extend:'Ext.panel.Panel',
	...
	constructor: function(config){
		this.callParent([Ext.apply(config,{
			title:'Reasoner Network view',
			gridUrl: NV.mplane.gui.Main.urls.gridUrl,
			runUrl : NV.mplane.gui.Main.urls.reasonerNetViewUrl,
			layout:{
				type:'fit'
			},
			closable:true,
	...
	Ext.Ajax.request({
				url : "/gui/reasoner/netview",
				success : function(response) {
					var result =  Ext.decode(response.responseText);
					var container = document.getElementById(targetID+'-body');
					data = {
						nodes: nodes
						,edges: edges
					};
		...
		window.setInterval(function(){
				self.setLoading("Updating network status");
				Ext.Ajax.request({
				url : "/gui/reasoner/netview",
				success : function(response) {
					var result =  Ext.decode(response.responseText);
...
Ext.ClassManager.addNameAliasMappings({
  ...
  "NV.mplane.gui.tab.ReasonerNetViewTab": [],
